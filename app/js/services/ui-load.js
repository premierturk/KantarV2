...
angular.module('ui.load', [])
    ...
    .service('kendoExt', function ($state, $window, $http, $log, SweetAlert, $timeout, $linq, $sessionStorage, $q, $localStorage, $rootScope) {
        ...
        var addSessionHistory = function (url, type) {
            var islem = $linq.Enumerable().From($sessionStorage.history)
                .Where(function (x) {
                    return x.url === url && (x.response === null || moment(x.request).add(3, 's') > moment());
                })
                .FirstOrDefault();
                ...
                transport: {
                    read: {
                        ...
                        beforeSend: function (req) {
                            ...
                            if (angular.isDefined($localStorage.user))
                                req.setRequestHeader('Authorization', 'Basic ' + $localStorage.user.authtoken);
                        }
                    }
                },
                error: function (e) {
                    if (e.errorThrown == "Unauthorized")
                        ...
                        SweetAlert.swal("Hata oluştu", "Sistem yöneticinizle görüşünüz.", "error");
                }
            });
        ...
        this.datasource = function (url, columns, onChange, onDataBound, onDataBinding, agg) {
        ...
        this.datasourceSF = function (url, columns, onChange, onDataBound, onDataBinding, agg, sort) {
                    ...
                    transport: {
                        read: {
                            ...
                            beforeSend: function (req) {
                                ...
                                req.setRequestHeader('Authorization', 'Basic ' + user.authtoken);
                            }
                        },
                        parameterMap: function (data, type) {
                            if (type == "read") {
        ...
        this.ddloptions = function (url, placeholder, dataTextField, dataValueField, change, error, dataBound) {
        ...
        this.ConvertToOptions = function (data, placeholder, dataTextField, dataValueField, change, error, dataBound) {
        ...
        function ColShowHide(e) {
                ...
                angular.forEach(kendoGrid.columns, function (item, key) {
                    if (item.columns)
                        angular.forEach(item.columns, function (itemSub, keySub) {
                            var col = $linq.Enumerable().From($localStorage.columnChose)
                                .Where(function (x) {
                                    return x.state == $state.current.name && x.column == itemSub.field
                                }).FirstOrDefault();

                            if (col == null) return;
                            if (col.isshow)
                                kendoGrid.showColumn(itemSub.field);
                            else
                                kendoGrid.hideColumn(itemSub.field);
                        });
                    else {
                        var col = $linq.Enumerable().From($localStorage.columnChose)
                            .Where(function (x) {
                        ...
                        if (col == null) return;
                        if (col.isshow)
                            kendoGrid.showColumn(item.field);
                        else
                            kendoGrid.hideColumn(item.field);
                    }
                });
            }
            ...
            angular.forEach($rootScope.colums, function (value, key) {
                ...
                angular.forEach(value.columns, function (valuesub, keysub) {
                    if (valuesub.hidden == true)
                        valuesub.isShow = false;
                    else
                        valuesub.isShow = true;
                });
            });
            ...
            $rootScope.changeColState = function (item) {
                ...
                var s = $linq.Enumerable().From($localStorage.columnChose)
                    .Where(function (x) {
                        return x.state == $state.current.name && x.column == item.field
                    }).FirstOrDefault();
